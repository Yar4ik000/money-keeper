...
- Включи **режим выбора** (зажми транзакцию) → выбери несколько → удали пакетом. Пакетное удаление требует подтверждения («Удалить выбранные операции?»).
...
**Настройки → Уведомления → Разрешения в системе** — открывает системные настройки Android для MoneyKeeper. Там можно проверить/включить `POST_NOTIFICATIONS`, управлять категориями уведомлений, разрешить звук и вибрацию.

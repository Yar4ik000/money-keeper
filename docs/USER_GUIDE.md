...
| Список | **История / Аналитика** | Полная история операций с фильтрами, графики, разбивка по категориям и счетам |
| График | **Прогнозы** | Прогноз балансов с учётом регулярных платежей и вкладов |
| Шестерёнка | **Настройки** | Категории, бюджеты, безопасность, экспорт, резервные копии |

Кнопка **«+»** (FAB) в центре нижней панели открывает форму добавления операции.
...
- Сумма слева — потрачено за текущий период, справа — лимит

...
| 107 | 1.4.0 | 2026-04-28 | История событий вклада + DAILY-сегментация; время у операций (DB v8→9); трёхуровневые категории + rollup в аналитике; разбивка по счетам в аналитике; прогноз cash-flow + сценарии + повторяющиеся запланированные события; fix restore backup из старых версий (user_version); fix капитализации при открытом приложении; время в «последних» операциях |
| 108 | 1.4.1 | 2026-04-29 | fix: краш при скролле в деталях счёта когда id события вклада совпадает с id транзакции (дубликат ключа в LazyColumn) |

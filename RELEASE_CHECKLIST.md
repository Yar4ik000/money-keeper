...
- [ ] **`BackupRestoreVersionChainTest` зелёный?**
  Тест автоматически прогоняет полный цикл бекап→рестор→бекап для **каждой исторической
  версии БД** (от v2 до `AppDatabase.VERSION`). Он расширяется сам при bump VERSION —
  дополнительный код не нужен, если структура таблицы `accounts` не менялась.
  Если менялась — обновить `INSERT_ACCOUNT_FOR_ALL_VERSIONS` в
  `BackupRestoreVersionChainTest.kt`.
...
| 106 | 1.3.3 | 2026-04-26 | Inline-поиск в истории операций (BasicTextField в TopAppBar); фильтр по диапазону сумм; валюта KZT (₸); 32 иконки категорий, 18 иконок счетов; блокировка переводов между разными валютами |
